...
internal sealed class GlobalCache : SerializableFile {
	...
	private static string SharedFilePath => Path.Combine(SharkFarm.SharedInfo.ConfigDirectory, $"{nameof(SteamTokenDumper)}.cache");
	...
	internal static async Task<GlobalCache?> Load() {
		...
		try {
			...
			if (string.IsNullOrEmpty(json)) {
				ASF.ArchiLogger.LogGenericError(SharkFarm.Localization.Strings.FormatErrorIsEmpty(nameof(json)));
	...
	internal void UpdateDepotKey(SteamApps.DepotKeyCallback depotKeyResult) {
		...
		if (!IsValidDepotKey(depotKey)) {
			ASF.ArchiLogger.LogGenericWarning(SharkFarm.Localization.Strings.FormatErrorIsInvalid(nameof(depotKey)));
