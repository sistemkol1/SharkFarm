...
using SharkFarm.OfficialPlugins.SteamTokenDumper.Data;
using SharkFarm.OfficialPlugins.SteamTokenDumper.Localization;
using SharkFarm.Plugins;
using SharkFarm.Plugins.Interfaces;
using SharkFarm.Steam;
using SharkFarm.Steam.Interaction;
using SharkFarm.Storage;
using SharkFarm.Web;
using SharkFarm.Web.Responses;
using SteamKit2;

namespace SharkFarm.OfficialPlugins.SteamTokenDumper;
...
#pragma warning restore CA5394 // This call isn't used in a security-sensitive manner
		...
		try {
			...
			foreach (uint[] appIDsThisRound in appIDsToRefresh.Chunk(Bot.EntriesPerSinglePICSRequest)) {
				...
				if (response.Results == null) {
					bot.ArchiLogger.LogGenericWarning(SharkFarm.Localization.Strings.FormatWarningFailedWithError(nameof(response.Results)));
	...
	private static string? ResponseRefreshManually(EAccess access, Bot bot) {
		...
		if (access < EAccess.Master) {
			return access > EAccess.None ? bot.Commands.FormatBotResponse(SharkFarm.Localization.Strings.ErrorAccessDenied) : null;
		}

		if (GlobalCache == null) {
			return bot.Commands.FormatBotResponse(SharkFarm.Localization.Strings.FormatWarningFailedWithError(nameof(GlobalCache)));
		}
		...
		return bot.Commands.FormatBotResponse(SharkFarm.Localization.Strings.Done);
	}
	...
	private static string? ResponseRefreshManually(EAccess access, string botNames, ulong steamID = 0) {
		...
		if ((bots == null) || (bots.Count == 0)) {
			return access >= EAccess.Owner ? Commands.FormatStaticResponse(SharkFarm.Localization.Strings.FormatBotNotFound(botNames)) : null;
		}
		...
		if (GlobalCache == null) {
			return Commands.FormatStaticResponse(SharkFarm.Localization.Strings.FormatWarningFailedWithError(nameof(GlobalCache)));
		}
		...
		return Commands.FormatStaticResponse(SharkFarm.Localization.Strings.Done);
	}
	...
	private static async Task SubmitData(CancellationToken cancellationToken = default) {
		...
		try {
			...
			if (response == null) {
				ASF.ArchiLogger.LogGenericWarning(SharkFarm.Localization.Strings.WarningFailed);
			...
			if (response.StatusCode.IsClientErrorCode()) {
				ASF.ArchiLogger.LogGenericWarning(SharkFarm.Localization.Strings.FormatWarningFailedWithError(response.StatusCode));
...
#pragma warning restore CA5394 // This call isn't used in a security-sensitive manner
			...
			if (response.Content is not { Success: true }) {
				ASF.ArchiLogger.LogGenericError(SharkFarm.Localization.Strings.WarningFailed);

				return;
			}

			if (response.Content.Data == null) {
				ASF.ArchiLogger.LogGenericError(SharkFarm.Localization.Strings.FormatErrorIsInvalid(nameof(response.Content.Data)));
